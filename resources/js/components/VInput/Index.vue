...
const props = defineProps({
    ...
    prefix: bool().def(false),
    tooltipBg: oneOf(["dark", "white"]).def("dark"),
    position: oneOf(["top", "bottom", "left", "right"]).def("top"),
});
...
<template>
    <div>
        <div
            class="mb-1"
            :class="{ 'flex items-center  my-auto': tooltip, block: !tooltip }"
            v-if="label"
        >
            <label
        ...
        <div
            class="relative"
            :class="{ 'flex justify-between items-center': prefix }"
        >
            <slot name="icon" />
            <input
                :id="placeholder"
