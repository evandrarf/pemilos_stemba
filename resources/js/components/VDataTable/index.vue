    ...
    <div class="overflow-y-visible ">
        ...
        <div class="overflow-x-auto px-3 no-scrollbar" 
            ...
            <table class="table-auto w-full relative" :class="[{ 'min-h-[40vh] sm:min-h-[50vh]': isLoading }, { 'border-separate border-spacing-0': freezeTable }]">
                <!-- Table header -->
                <thead
                    class="text-xs font-semibold capitalize text-slate-500 bg-[#F8FAFC] border-t border-b border-slate-200" :class="{'header': freezeTable}">
                    <tr>
                        <th v-for="(head, index) in heads" :key="index"
                            class="px-4 h-10 whitespace-nowrap border-t border-b border-slate-200" :class="{'fixed-header': freezeTable && index === 0}">
